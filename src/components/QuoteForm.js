...
import { Plus, Trash2, Save, X, FileText, Search, Percent, Calculator, ChevronRight, ChevronDown, Package, AlertCircle, Layers } from 'lucide-react';
import DeleteButton from './DeleteButton';

export default function QuoteForm({ action, initialData, customers, products, categories = [] }) {
  ...
  // Sync state if initialData changes (e.g. after save/revalidation)
  useEffect(() => {
    if (initialData) {
      setCustomerId(initialData.customerId || '');
      setStatus(initialData.status || 'Draft');
      setVatRate(initialData.vatRate ?? 20);
  ...
  // Pre-process items and BOM items to find their categories if they exist
  useEffect(() => {
    if (products.length > 0 && items.length > 0) {
      ...
      const enhancedItems = items.map(item => {
        ...
        // Process main item
        if (!newItem.mainGroupId && newItem.productId) {
          const product = products.find(p => p.id === newItem.productId);
          if (product && product.categoryId) {
            const cat = categories.find(c => c.id === product.categoryId);
            if (cat) {
              itemChanged = true;
              const parent1 = cat.parentId ? categories.find(c => c.id === cat.parentId) : null;
              const parent2 = parent1 && parent1.parentId ? categories.find(c => c.id === parent1.parentId) : null;

              if (parent2) {
                newItem.mainGroupId = parent2.id;
                newItem.subgroupId = parent1.id;
                newItem.subSubgroupId = cat.id;
              } else if (parent1) {
                newItem.mainGroupId = parent1.id;
                newItem.subgroupId = cat.id;
                newItem.subSubgroupId = '';
              } else {
                newItem.mainGroupId = cat.id;
                newItem.subgroupId = '';
                newItem.subSubgroupId = '';
              }
            }
        ...
        // Process BOM items
        if (newItem.bomItems && newItem.bomItems.length > 0) {
          const updatedBom = newItem.bomItems.map(bom => {
            if (!bom.mainGroupId && bom.productId) {
              ...
              if (product && product.categoryId) {
                ...
                if (cat) {
                  ...
                  if (p2) return { ...bom, mainGroupId: p2.id, subgroupId: p1.id, subSubgroupId: cat.id };
                  if (p1) return { ...bom, mainGroupId: p1.id, subgroupId: cat.id, subSubgroupId: '' };
                  return { ...bom, mainGroupId: cat.id, subgroupId: '', subSubgroupId: '' };
                }
              }
            }
            return bom;
      ...
      if (item.id === id) {
        ...
        if (field === 'mainGroupId') {
          ...
          if (product) {
            ...
            // Clear BOM if we changed to a non-BOM product
            if (!product.isBOM) {
              updated.bomItems = [];
  ...
  const isBOMProduct = (productId) => {
    ...
    // Check if category or any of its parents have bomEnabled set to true
  ...
  return (
    <form action={action} className="card" style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: '1rem' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
          <label style={{ fontWeight: '700', fontSize: '0.875rem' }}>Valid Until</label>
          <input type="date" name="validUntil" value={validUntil} onChange={(e) => setValidUntil(e.target.value)} />
        </div>
      </div>
      ...
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
          <h3 style={{ margin: 0, fontSize: '1rem', fontWeight: '700' }}>Line Items</h3>
        </div>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', minWidth: '1100px', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {items.length === 0 ? (
                <tr>
                  <td colSpan="8" style={{ padding: '2rem', textAlign: 'center', color: 'var(--muted-foreground)', fontSize: '0.875rem' }}>
                    No items added.
                  </td>
                </tr>
              ) : (
                items.map((item, index) => {
                  ...
                  return (
                    <tr key={item.id}>
                      <td colSpan="8" style={{ padding: '0.2rem 0' }}>
                        <div style={{ 
                        ...
                        onMouseEnter={(e) => {
                          ...
                          e.currentTarget.style.boxShadow = '0 4px 8px -2px rgb(0 0 0 / 0.2)';
                          e.currentTarget.style.borderColor = 'var(--primary)';
                        }}
                        onMouseLeave={(e) => {
                          e.currentTarget.style.transform = 'translateY(0)';
                          e.currentTarget.style.boxShadow = '0 2px 4px -1px rgb(0 0 0 / 0.1)';
                          ...
                          <div style={{ flex: 1, padding: '0.4rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                            <div style={{ display: 'flex', gap: '0.4rem', alignItems: 'center' }}>
                              <select 
                                ...
                                style={{ width: '150px', fontSize: '0.75rem', padding: '0.3rem' }}
                              >
                            ...
                            {/* CUSTOM DESCRIPTION (Only if BOM) */}
                            {isBom && (
                          ...
                          <div style={{ width: '60px', padding: '0.4rem' }}>
                            <input 
                              ...
                              step="any"
                              min="0"
                              style={{ width: '100%', fontSize: '0.8125rem', padding: '0.3rem' }}
                            />
                          </div>
                        ...
                        {isBom && (
                          <div style={{ 
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: item.isCollapsed ? '0' : '0.5rem' }}>
                              <div 
                                onClick={() => updateItem(item.id, 'isCollapsed', !item.isCollapsed)}
                                style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', color: '#334155' }}
                              >
      ...
      <div style={{ display: 'flex', gap: '1rem', justifyContent: 'space-between', marginTop: '1rem', alignItems: 'center' }}>
        <button type="button" onClick={addItem} className="btn-secondary" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.6rem 1.5rem' }}>
          <Plus size={18} /> Add Item
        </button>
        
        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
          <Link href="/quotes" className="btn-secondary" style={{ padding: '0.6rem 1.5rem', background: '#e2e8f0', color: '#475569' }}>
            Close
          </Link>
